...
- **HTTP/2 compat** (`src/lib/http2Compat.ts`) — Unless `http2: false` is set, the secure server is `http2.createSecureServer({ allowHTTP1: true })`. Express swaps the prototype of every request/response to its HTTP/1-based one, which breaks `Http2ServerRequest`/`Http2ServerResponse` (crashes the process on the first body read). `adaptHttp2Request()` pins the HTTP/2 prototype members (and HTTP/1-only members resolved via the HTTP/2 chain) onto each instance, fills `host` from `:authority`, adds a `transfer-encoding` hint for bodies without `content-length` (type-is/body-parser skip them otherwise) and drops HTTP/1 connection headers on the response (Node throws on them out of `res.end()`). `createSecureServer()` in `webServer.ts` also closes open HTTP/2 sessions on `server.close()` — otherwise the close callback never fires.
